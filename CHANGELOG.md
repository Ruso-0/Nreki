...
Docs-only patch. Rewrites the `nreki init` templates to reflect v10.15.0 features and adds an AGENTS.md template for non-Claude agent runtimes.

### Added

- **AGENTS.md template**: new `templates/AGENTS.md`, generated by `nreki init` alongside CLAUDE.md. Same body, header targeted at AI coding agents consuming NREKI via MCP (Codex, Gemini, others). `nreki deinit` cleans it.
- **getAgentsMdContent()** helper + mirrored init/deinit blocks in src/index.ts. AGENTS.md is appended if the file exists without the NREKI marker, created fresh otherwise.

### Changed

- **CLAUDE.md template** rewritten as "NREKI ACTIVE (IMMUNE SYSTEM)" manifest with v10.15.0 features: `fast_grep` as the default exact-search tool, 80L guillotine for batch_edit payload decomposition, patch mode mandate for symbols >40 lines, ACID rules for multi-patching, TTRD (Type Discipline) bounties, engram memory anchoring.

### Notes

- No code/runtime behavior changes. Safe patch bump from v10.15.0. 845/845 tests pass unchanged.
