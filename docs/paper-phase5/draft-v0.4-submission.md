...
While NREKI's architecture is engineered to withstand 14GB-class monolithic scale (e.g., our VS Code boot benchmarks), the empirical retrieval evaluation rigorously bounds itself to the PolyBench-Verified distribution, peaking at ~500k LOC frameworks like Material-UI.
...
Figure 1: Late-Fusion T-RAG retrieval pipeline architecture. Stage 1 (parallel retrieval): NREKI Type Ledger extracts topological signals from AST + cross-file dependencies; standalone BM25 performs lexical matching. Both retrievers produce candidate pools of topK*4=40 files each. Stage 2 (Reciprocal Rank Fusion): file-level paths are scored via 1/(60 + rank) summed across retrievers, with deterministic lexicographic tie-breaking. Stage 3 (chunk mapping): top-K=10 fused files are augmented with chunks preferentially from NREKI's foveal compression output, falling back to BM25 chunks for BM25-exclusive files. Local MCP protocol transports results to coding agent. NREKI standalone is the efficiency-dominant local retriever; Hybrid RRF is the recall-dominant local retriever.
...
| Comparison | Metric | Delta | CI95 low | CI95 high | p-value | Significant (alpha=0.01) |
|---|---|---:|---:|---:|---:|:---:|
| hybrid - nreki | FHR | +0.152 | +0.071 | +0.232 | 0.0062 | ** |
| hybrid - nreki | CLR | +0.081 | +0.026 | +0.140 | 0.0004 | ** |
| hybrid - nreki | R@1 | +0.111 | +0.030 | +0.192 | 0.0062 | ** |
| hybrid - nreki | R@5 | +0.101 | +0.010 | +0.192 | 0.0388 | * |
| hybrid - nreki | MRR | +0.111 | +0.047 | +0.176 | 0.0002 | ** |
| hybrid - bm25 | FHR | +0.192 | +0.091 | +0.293 | 0.0010 | ** |
| hybrid - bm25 | CLR | +0.129 | +0.058 | +0.204 | 0.0002 | ** |
| hybrid - bm25 | R@1 | +0.141 | +0.051 | +0.232 | 0.0010 | ** |
| hybrid - bm25 | R@5 | +0.172 | +0.071 | +0.273 | 0.0012 | ** |
| hybrid - bm25 | MRR | +0.156 | +0.081 | +0.233 | 0.0002 | ** |

** denotes significance surviving Bonferroni correction (`alpha_corrected = 0.05/5 = 0.01`). * denotes single-test significance (`p<0.05`) not surviving multiple-hypothesis correction. Marginal results like Hybrid-NREKI R@5 (`p=0.0388`) are strictly reported as non-significant to guard against Type I error inflation.
...
| Comparison | Metric | Delta | CI95 low | CI95 high | p-value | Significant (alpha=0.01) |
|---|---|---:|---:|---:|---:|:---:|
| nreki - bm25 | FHR | +0.040 | -0.081 | +0.162 | 0.5512 |  |
| nreki - bm25 | CLR | +0.049 | -0.038 | +0.135 | 0.5700 |  |
| nreki - bm25 | R@1 | +0.030 | -0.061 | +0.121 | 0.5512 |  |
| nreki - bm25 | R@5 | +0.071 | -0.051 | +0.192 | 0.2538 |  |
| nreki - bm25 | MRR | +0.045 | -0.042 | +0.131 | 0.3022 |  |

** denotes significance surviving Bonferroni correction (`alpha_corrected = 0.05/5 = 0.01`). * denotes single-test significance (`p<0.05`) not surviving multiple-hypothesis correction.
...
This topological bypass is representative of the four distinct tasks in our evaluation where the Late-Fusion Hybrid achieved Rank-1 retrieval despite NREKI Standalone completely failing to locate the target within the Top-10. A related broader pattern covers ten distinct tasks where hybrid achieved Rank-1 retrieval despite neither standalone retriever placing a strict-source file at Rank 1.
...
### 5.5 The Retrieval-Resolution Disconnect
...
and methodology are archived in supplementary materials
(`corpus-measurements-v11.0.1.json`).
...
The voyage-code-3 paired subset is limited to N=15 because the available API quota was exhausted during Phase 5 evaluation. Voyage's documented free tier for voyage-code-3 is 200 million tokens per account [46]. This should be read both as a limitation and as an operational finding. It prevents a larger paired accuracy comparison against the dense baseline, but it also demonstrates the practical fragility of cloud dense retrieval under repository-scale workloads. Reproducing the Voyage analysis requires a fresh free-tier quota or paid quota.
...
NREKI is released as open-source software under [License to be disclosed post-review for double-blind compliance]. Source code and evaluation scripts are provided as anonymous supplementary materials (see supplementary archive `code-anonymous.zip`). The hybrid-runner Late-Fusion implementation resides at `scripts/eval-phase5/runners/hybrid-runner.ts`; deterministic tie-breaking is implemented at lines 78-81. The `VOYAGE_MODEL` constant is verified at `scripts/eval-phase5/runners/voyage-runner.ts` line 32. Public release upon paper acceptance is planned under Apache License 2.0.
...
Bootstrap resampling was conducted over 10,000 iterations using fixed `random_seed=42`. Implementation: `scripts/eval-phase5/bootstrap-rk-mrr.ts`. Bonferroni correction: `alpha_corrected = 0.05 / 5 = 0.01` per metric within each pairwise comparison.

### 10.5 Voyage Paired Subset Reproduction

Voyage paired N=15 analysis is implemented in `scripts/eval-phase5/voyage-paired-analysis.ts`. Reproduction requires `VOYAGE_API_KEY`. The voyage-code-3 free-tier quota of 200 million tokens per account [46] was exhausted during Phase 5 evaluation, so reproduction requires either paid quota or a fresh free-tier quota.
...
[21] Tree-sitter. "tree-sitter/tree-sitter: An incremental parsing system for programming tools." GitHub repository, accessed May 2026. [Repository URL omitted for double-blind review].

[22] GitHub Docs. "About repository languages." GitHub Docs, accessed May 2026. [Documentation URL omitted for double-blind review].

[23] GitHub Linguist. "github-linguist/linguist." GitHub repository, accessed May 2026. [Repository URL omitted for double-blind review].

[24] Microsoft TypeScript. "Using the Compiler API." TypeScript Wiki, accessed May 2026. [Documentation URL omitted for double-blind review].
...
[31] ZillizTech. "Claude Context MCP Server." GitHub repository, accessed May 2026. [Repository URL omitted for double-blind review].
...
[40] SkillsIndex. "gitnexus MCP Server - Security Score, Reviews & Alternatives." Accessed May 2026. https://skillsindex.dev/tools/gitnexus/

[41] code-review-graph [Source code]. GitHub. 2026. [Repository URL omitted for double-blind review].

[42] CodeGraphContext [Source code]. GitHub. 2026. [Repository URL omitted for double-blind review].

[43] codebase-memory-mcp [Source code]. GitHub. 2026. [Repository URL omitted for double-blind review].

[44] PulseMCP. "Codebase Memory MCP Server by DeusData." Accessed May 2026. https://www.pulsemcp.com/servers/deusdata-codebase-memory

[45] Claude Context MCP Server [Source code]. GitHub. 2026. [Repository URL omitted for double-blind review].

[46] Voyage AI. "Pricing." Voyage AI documentation, accessed May 2026. https://docs.voyageai.com/docs/pricing
