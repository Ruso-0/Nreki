...
![Figure 1 Architecture Diagram Placeholder](figures/figure1-architecture-placeholder.png)

[FIGURE 1 ARCHITECTURE DIAGRAM HERE - to be rendered by human/figure tool post-draft]
...
![Figure 2 Pareto Frontier Placeholder](figures/figure2-pareto-frontier-placeholder.png)

[FIGURE 2 PARETO FRONTIER SCATTER PLOT HERE - to be rendered by human/figure tool post-draft]
...
| Comparison | Metric | Delta | CI95 low | CI95 high | p-value | Significant |
|---|---|---:|---:|---:|---:|:---:|
| hybrid - nreki | FHR | +0.152 | +0.071 | +0.232 | 0.0002 | Y |
| hybrid - nreki | CLR | +0.081 | +0.026 | +0.140 | 0.0026 | Y |
| hybrid - nreki | R@1 | +0.111 | +0.030 | +0.192 | 0.0068 | Y |
| hybrid - nreki | R@5 | +0.101 | +0.010 | +0.192 | 0.0366 | Y |
| hybrid - nreki | MRR | +0.111 | +0.047 | +0.176 | 0.0002 | Y |
| hybrid - bm25 | FHR | +0.192 | +0.091 | +0.293 | 0.0002 | Y |
| hybrid - bm25 | CLR | +0.129 | +0.058 | +0.204 | 0.0004 | Y |
| hybrid - bm25 | R@1 | +0.141 | +0.051 | +0.232 | 0.0036 | Y |
| hybrid - bm25 | R@5 | +0.172 | +0.071 | +0.273 | 0.0018 | Y |
| hybrid - bm25 | MRR | +0.156 | +0.081 | +0.233 | 0.0002 | Y |
...
| Comparison | Metric | Delta | CI95 low | CI95 high | p-value | Significant |
|---|---|---:|---:|---:|---:|:---:|
| nreki - bm25 | FHR | +0.040 | -0.081 | +0.162 | 0.5724 | N |
| nreki - bm25 | CLR | +0.049 | -0.038 | +0.135 | 0.2768 | N |
| nreki - bm25 | R@1 | +0.030 | -0.061 | +0.121 | 0.5600 | N |
| nreki - bm25 | R@5 | +0.071 | -0.051 | +0.192 | 0.2704 | N |
| nreki - bm25 | MRR | +0.045 | -0.042 | +0.131 | 0.3114 | N |
...
### 5.4 Retrieval Is Not Resolution
...
### 5.5 Role of Adversarial Review

The adversarial review process affected the technical result. It caught OOM teardown risk, simulation/implementation granularity mismatch, truncation horizon asymmetry, and nondeterministic tie-breaking. Without those checks, the paper could have reported a more convenient but less true result. The final result is stronger because the failed paths are documented and the claims are narrower. This is a methodological argument as much as an engineering one: high-variance AI systems need adversarial audit loops before their metrics become paper claims.
...
Retrieval systems for coding agents can increase developer productivity, but they can also accelerate unsafe edits if agents treat retrieved context as complete truth. This work should be used as an evidence-delivery component, not as an autonomous permission system. Better retrieval makes it more likely that a model sees relevant files; it does not remove the need for tests, code review, security checks, or human accountability in high-risk systems.

Local-first retrieval has privacy and reliability benefits because repositories need not be uploaded to cloud embedding services for every run. That advantage matters for proprietary codebases and regulated environments. It also shifts responsibility to local operators: they must secure indexes, logs, and generated artifacts because local does not automatically mean safe. The evaluation artifacts in this paper contain benchmark repositories and task metadata, not private user code.

## 10. Reproducibility Statement

All evaluation artifacts supporting this work are designed for independent reproduction.

Dataset: PolyBench-Verified is available at Hugging Face as `AmazonScience/SWE-PolyBench_Verified`. The N=99 evaluable subset excludes only tasks with empty `strict_src` ground truth. The full Sprint 6.4 JSONL contains 100 records and preserves the excluded empty-ground-truth task for auditability.

Source code: NREKI is versioned in `package.json` as `@ruso-0/nreki` v11.0.0 and licensed Apache-2.0. The repository URL in the package metadata is `https://github.com/Ruso-0/nreki`. The hybrid-runner late-fusion implementation is located at `scripts/eval-phase5/runners/hybrid-runner.ts`; deterministic tie-breaking is implemented at lines 78-81 in the current working tree. The Voyage runner verifying voyage-code-3 model invocation is located at `scripts/eval-phase5/runners/voyage-runner.ts`, with `VOYAGE_MODEL = "voyage-code-3"` at line 32.

Evaluation orchestration: the full N=99 reproduction command is:
...
The Sprint 6.4 measured wall clock was 8020.14 seconds, approximately 2h13m on the evaluated commodity hardware. The command wrote `results-c4b-v3-with-hybrid.jsonl` and completed with exit code 0.

Raw artifacts: Sprint 6.4 tribunal output JSONL is referenced as artifact [40]. The corrected voyage-code-3 paired-analysis input is referenced as artifact [41]. Bootstrap CI95% computation and paired analysis are implemented in `scripts/eval-phase5/bootstrap-rk-mrr.ts` and `scripts/eval-phase5/voyage-paired-analysis.ts`. The p-values in Tables 2 and 3 are computed from 10,000 paired bootstrap resamples using the directional-tail rule described in the Results section. Random seed control is documented in source for deterministic reproduction.
...
[48] M. Vogel, F. Meyer-Eschenbach, S. Kohler, E. Grunewald, and F. Balzer. "Codebase-Memory: Tree-Sitter-Based Knowledge Graphs for LLM Code Exploration via MCP." arXiv:2603.27277, 2026. https://arxiv.org/abs/2603.27277
