...
- If `search` returns "No semantic results found", **read the fallback list in the response** — it names `fast_grep` (exact substring, RAM-resident) and broaden-query before reaching for the host agent's grep.
